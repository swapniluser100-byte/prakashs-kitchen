...
const FORM_ENDPOINT =
  "https://script.google.com/macros/s/AKfycbzkixZjFua-IlFbYnPb1m4wlTrrBljN7W0vLJk9uj30Q2D-mBzNSL715DbtNOzYpA90LA/exec";
...
function getValidatedAddress() {
  ...
  if (!address) {
    alert(
      "Please enter a delivery address — it's required to complete your order.",
    );
    addressInput.focus();
    return null;
  }
...
document
  .getElementById("confirmPaidBtn")
  .addEventListener("click", function () {
    const address = getValidatedAddress();
    if (!address) return;

    const amount =
      document.getElementById("upiAmount").value || pendingOrder.total;
